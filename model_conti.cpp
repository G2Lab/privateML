...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    double input_intercept = CSVtoConst(intercept);
    uint64_t input_b;
    if (input_intercept > 0)  {
        input_b = input_intercept * scaling;
    } else {
        input_b = plain_modulus + input_intercept * scaling;
    }
    vector<int64_t> weights = ScaleVector(input_weights, scaling);
    cout << "intercept : " << input_intercept << " -> " << input_b << endl;
    
    cout << "\nEncoding weights..." << endl;
    vector<int64_t> vec4(slot_count);
    vector<Plaintext> w(num_full_ctxt + 1); 
    Plaintext w_intercept;
    for (int j = 0; j < w.size(); j++) {
        batch_encoder.encode(vec4, w[j]);
    }
    batch_encoder.encode(vec4, w_intercept);
    ...
    if (input_b > 0)
    {
        w_intercept.data()[0] = input_b;
    }
    else
    {
        w_intercept.data()[0] = plain_modulus - input_b;
    }

    vector<Ciphertext> cipherweight(num_full_ctxt + 2);
    cout << "\nEncrypting model..." << endl;
    for (int j = 0; j < num_full_ctxt+1; j++) {
        encryptor_pub.encrypt(w[j], cipherweight[j]);
    }
    encryptor_pub.encrypt(w_intercept, cipherweight[num_full_ctxt+1]);
    ...
    vector<Ciphertext> weightcipher = LoadVec(modelname, context, num_full_ctxt + 2);
    ...
    for (int sample = 0; sample < num_samples; sample++)
    {
        ...
        evaluator.add_inplace(wg[0], weightcipher[num_full_ctxt + 1]);
